...
(defun compilation (nomFichier &optional dest)
...
(defun affiche-code-ligne (code ligne)
  (if (null code)
    NIL
    (progn
      (print  (write-to-string (car code)))
...
(defun compil-expr (expr &optional env)
  (cond
  	...
  	((symbolp expr) (compil-LIT expr env))
...
(defun compil-LIT (LIT  &optional env)
	(let ((lib (assoc LIT env)))
		(if lib
			(append
				`((MOVE FP R0))
				`((SUB (LIT ,(cdr lib)) R0))
			...
			`((MOVE (LIT ,LIT) RO)) ; pour la gestion des let
		)
	)
)
